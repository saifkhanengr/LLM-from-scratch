...
def clean_response(generated_text):
    ...
    # Print the prompt part (everything up to "Response:")
    if "Response:" in text:
        prompt_part = text.split("Response:", 1)[0] + "Response:"
    else:
        prompt_part = ""  # fallback if "Response:" not found

    print("=======================================")
    print(f"{prompt_part.strip()}")

    # remove everything before/including "Response:" for model response
    if "Response:" in text:
